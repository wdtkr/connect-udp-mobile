...
static void receiveCFunction(const char *message, ssize_t size, int type) {
    if (globalReceiveBlock != nil) {
        // message を NSString に変換し、globalReceiveBlock を呼び出します。
        // size と type は使われていない場合、無視されるか、適宜使用してください。
